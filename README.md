...
| **Dashboard** | Embedded Vue.js UI served from the coordinator binary (TLS default port `443`, configurable) |
...
| **Credential profiles** | AES-256-GCM encrypted; SMB/SSH credential management per job |
| **Job cancellation** | Cancel running jobs via API and dashboard; status flow through WebSocket |
| **Template management** | Reusable job templates; scheduled template execution |
| **Audit logging** | Command audit trails; user action history; non-whitelisted program detection |
| **Rollback** | One-version rollback for coordinator and agents |
| **Delete agents** | Admin-only agent deletion with running-job guard |
...
```bash
coordinator start
# Dashboard available at https://localhost (or http://localhost:<port> if TLS is disabled)
```
...
| Version | Highlights |
|---|---|
| v0.5.1 | Royal Purple theme; WebSocket origin validation hardening; secret protection patterns |
| v0.5.0 | Real-time job progress tracking; ProgressBar UI component; job_runs auto-creation |
| v0.3.0 | RBAC, JWT authentication, user/group management, federation HA, multi-coordinator failover, health monitoring |
| v0.2.4 | Alert history retention; missed schedule dedup; federation lag polling |
| v0.2.3 | Notifications: webhook (HMAC-signed), Email (SMTP), Slack, Microsoft Teams |
| v0.2.2 | Agent rollback tracking; ScheduleBuilder UI; credential profiles |
| v0.2.1 | Job cancellation; audit logging; command audit |
| v0.1.0 | Initial release — single binary, embedded dashboard, agent registration, cron-based jobs |
