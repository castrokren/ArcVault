...
func (s *Server) triggerScheduledJobs() {
	...
	for rows.Next() {
		var j resetJob
		if err := rows.Scan(&j.id, &j.agentID); err == nil {
			jobs = append(jobs, j)
		}
	}
	...
	for _, j := range jobs {
		_, err := s.db.Conn().Exec(
			`UPDATE jobs SET status = 'pending' WHERE id = ?`, j.id,
		)
		if err != nil {
			log.Printf("Scheduler: failed to reset job %s: %v", j.id, err)
			continue
		}
		log.Printf("Scheduler: reset job %s to pending", j.id)
		s.hub.Broadcast(Event{
			Type:    "job.updated",
			Payload: map[string]string{"id": j.id, "status": "pending"},
		})
		// Notify agent to poll
		if err := s.hub.SendToAgent(j.agentID, map[string]string{"type": "poll_now"}); err != nil {
			log.Printf("Scheduler: agent %s not connected via WS for reset job %s: %v", j.agentID, j.id, err)
		}
	}
}
...
func (s *Server) fireTemplate(t db.Template) {
	...
	if err := s.hub.SendToAgent(t.AgentID, map[string]string{"type": "poll_now"}); err != nil {
		log.Printf("Scheduler: agent %s not connected via WS for template %s: %v", t.AgentID, t.ID, err)
	}
}
