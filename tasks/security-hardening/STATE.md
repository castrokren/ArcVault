...
2. **`decryptCredentials()` fails silently** (`server/credentials.go:193`). Every error path returns `nil`, and both callers (`jobs.go:224`, `jobs.go:262`) do `if credentials != nil`. A wrong/missing key means the agent runs the backup with no credentials instead of failing loudly. Should log at minimum; arguably the job should refuse to dispatch.
...
6. **Delete `C:\ArcVault\service-run.bat`.** Dead, and actively misleading: it names a stale binary, and the credential key it exports is now redundant with the registry. It cost this session a wrong first move.
...
- `coordinator/server/credentials.go` — `loadCredentialKey()` (config first, then env), `decryptCredentials()` (silent nil on failure)
